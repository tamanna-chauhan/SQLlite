...
//    delete data
    ...
    public void onUpdate(String name){
        ...
        db.update("USER",contentValues,"Name=?",arr);

    }

    Cursor getUserByName(String name){

        Cursor cursor = getdata();

        String nameQuery = "select * from StudentDB where name = '" +name+ "'; ";
        SQLiteDatabase sqLiteDatabase = getReadableDatabase();
        Cursor  = sqLiteDatabase.execSQL(nameQuery);
        return  cursor;
